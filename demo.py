...
st.set_page_config(
        page_title="Conversational Recommendation - Demo",
        page_icon=":robot:"
)


st.header("Conversational Recommendation - Demo")
...
def get_text():
        input_text = st.text_input("Your turn: ", "", key="input")
        return input_text 
...
if __name__ == '__main__':
        ...
        if user_input!= "quit" and user_input!= "":
                loop= TrainLoop_fusion_gen(vars(args))
                met= loop.val(user_input)
                output["generated_text"] = met
