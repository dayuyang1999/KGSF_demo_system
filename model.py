...
class CrossModel(nn.Module):
    def __init__(self, opt, dictionary, is_finetune=False, padding_idx=0, start_idx=1, end_idx=2, longest_label=1):
        ...
        self.info_con_loss = nn.MSELoss(reduction='sum')
        self.info_db_loss = nn.MSELoss(reduction='sum')
    ...
    def save_model(self, is_finetune= False):
        if is_finetune== False:
            torch.save(self.state_dict(), 'saved_model/net_parameter_non_finetune.pkl')
        else:
            torch.save(self.state_dict(), 'saved_model/net_parameter_finetune.pkl')
        

    def load_model(self, is_finetune= False):
        if is_finetune== False:
            self.load_state_dict(torch.load('saved_model/net_parameter_non_finetune.pkl'))
        else:
            self.load_state_dict(torch.load('saved_model/net_parameter_finetune.pkl'))
